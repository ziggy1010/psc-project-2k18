...
clahe_adaboost = lambda x: clahe(x)
he_adaboost = lambda x: histogram_equalisation(x)
ln_adaboost = lambda x: linNormalization(x)
...
def calcError(weight1, weight2, weight3, f1, f2, f3): #error function
    ...
    for i in range(img_num):
        epsilon += ((f(img.flatten(), weight1, weight2, weight3, f1, f2, f3) - wanted_output1)**2)
        # print(epsilon)
...
def linCombination(img, f1, f2, f3): #linear combination of methods
    ...
    for weight1 in frange(0, 10, 0.1):
        # sys.stdout.write("%f%%\r" % 100*(weight1 - sys.argv[0]) / l)
...
print('kita') 
for fn in glob("greyscaled-pics/first_method/*"):
    img = cv.imread(fn, 0)
    img_arr = img.flatten()

    linCombination(img_arr, clahe(img), histogram_equalisation(img), linNormalization(img))